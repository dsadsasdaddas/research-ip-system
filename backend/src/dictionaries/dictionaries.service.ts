...
export class DictionariesService implements OnModuleInit {
  // 构造器为参数属性 + @InjectRepository 装饰器,istanbul 会把每个 @InjectRepository 算成虚假分支,无真实逻辑
  constructor(
    @InjectRepository(DictionaryType)
    private readonly typeRepo: Repository<DictionaryType>,
  ...
  async removeType(id: number): Promise<{ deleted: true; id: number }> {
    const type = await this.findTypeById(id);
    if (type.isSystem)
      throw new BadRequestException('系统内置字典类型不能删除');
  ...
  async createItem(dto: CreateDictionaryItemDto): Promise<DictionaryItem> {
    ...
    const exists = await this.itemRepo.findOne({
      where: { typeCode: dto.typeCode, value: dto.value },
    });
  ...
  async updateItem(
    ...
    if (dto.value && dto.value !== item.value) {
      const exists = await this.itemRepo.findOne({
        where: { typeCode: item.typeCode, value: dto.value },
      });
      if (exists) throw new ConflictException('该字典类型下 value 已存在');
    }
  ...
  async removeItem(id: number): Promise<{ deleted: true; id: number }> {
    const item = await this.findItem(id);
    if (item.isSystem)
      throw new BadRequestException('系统内置字典项不能删除，可停用');
  ...
  private async seedDefaults(): Promise<void> {
    const types: DictionaryTypeSeed[] = [
      ...
      {
        code: 'paper_included_type',
        name: '论文收录类型',
        scope: 'business',
        remark: 'SCI/EI/CSCD 等',
      },
      {
        code: 'paper_partition',
        name: '中科院分区',
        scope: 'business',
        remark: '论文分区',
      },
      {
        ...
        name: '专利类型',
        scope: 'business',
        remark: '发明/实用新型/外观设计/PCT',
      },
      {
        code: 'patent_legal_status',
        name: '专利法律状态',
        scope: 'business',
        remark: '申请中/授权/失效/驳回',
      },
      ...
      {
        code: 'transform_status',
        name: '转化节点',
        scope: 'business',
        remark: '合同签订/收款/开票/完成等',
      },
      {
        code: 'result_type',
        name: '成果类型',
    ...
    for (const type of types) {
      ...
      if (!exists)
        await this.typeRepo.save(
          this.typeRepo.create({ ...type, isSystem: true, isActive: true }),
        );
    }

    const items: DictionaryItemSeed[] = [
      {
        typeCode: 'secret_level',
        label: '公开',
        value: '公开',
        sortOrder: 1,
        color: 'info',
    ...
    for (const item of items) {
      const exists = await this.itemRepo.findOne({
        where: { typeCode: item.typeCode, value: item.value },
      });
      if (!exists)
