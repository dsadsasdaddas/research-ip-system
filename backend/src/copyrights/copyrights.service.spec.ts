...
import type { Copyright } from './entities/copyright.entity';
import { UserRole } from '../users/entities/user.entity';

describe('CopyrightsService', () => {
  let service: CopyrightsService;
  let repo: ReturnType<typeof mockRepository>;

  beforeEach(() => {
    repo = mockRepository();
    service = new CopyrightsService(repo as unknown as Repository<Copyright>);
  });
  ...
  describe('create', () => {
    it('注入 deptId/createUser(忽略前端传入值)并保存', async () => {
      ...
      await service.create(
        { name: '软著A', deptId: 999, createUser: 'hacker' },
  ...
  describe('findOne', () => {
    ...
    it('无 user → 直接返回', async () => {
      const item = { id: 1, secretLevel: '公开', deptId: 5 } as Copyright;
    ...
    it('sys_admin 全院可看 → 返回', async () => {
      const item = { id: 1, secretLevel: '涉密', deptId: 99 } as Copyright;
      repo.findOne.mockResolvedValue(item);
      await expect(
    ...
    it('secretLevel 为空按公开处理 + 部门匹配 → 返回', async () => {
      const item = { id: 1, secretLevel: null, deptId: 5 } as Copyright;
      repo.findOne.mockResolvedValue(item);
      await expect(
        service.findOne(1, mockAuthUser(UserRole.RESEARCHER, 5)),
      ).resolves.toBe(item);
  ...
  describe('update', () => {
    it('存在 → 合并(忽略 deptId/createUser)并保存', async () => {
      ...
      expect(repo.save).toHaveBeenCalled();
      expect((item as Record<string, unknown>).name).toBe('new');
