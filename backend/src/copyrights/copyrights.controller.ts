...
export class CopyrightsController {
  ...
  /**
   * 导出当前(可按 keyword 过滤的)软件著作权列表为 xlsx/csv,直接回传文件流下载。
   * POST /api/copyrights/export { format, keyword, columns }
   */
  @Post('export')
  async exportResource(
    ...
    const rows = await this.copyrightsService.exportResource(
      { keyword: dto.keyword },
      user,
    );
    ...
    await sendExportFile(res, {
      filename: `copyrights_${today}.${dto.format || 'xlsx'}`,
  ...
  findAll(
    ...
    return this.copyrightsService.findAll(
      {
        keyword,
