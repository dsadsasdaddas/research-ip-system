...
export class UsersService {
  ...
  async create(dto: CreateUserDto): Promise<PublicUser> {
    ...
    const user = this.repo.create({ ...dto, password: hash, deptId: dto.deptId ?? null, realName: dto.realName ?? null, email: dto.email ?? null });
  ...
  async remove(id: number): Promise<{ deleted: true; id: number }> {
    const user = await this.findById(id);
    if (user.username === 'admin') throw new BadRequestException('默认管理员不能删除');
    const sysAdminCount = await this.repo.count({ where: { role: UserRole.SYS_ADMIN, isActive: true } });
    if (user.role === UserRole.SYS_ADMIN && sysAdminCount <= 1) throw new BadRequestException('至少保留一个启用的系统管理员');
