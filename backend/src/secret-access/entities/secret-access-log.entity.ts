...
export class SecretAccessLog {
  ...
  @Column({ type: 'varchar', name: 'business_type', length: 50, comment: '业务类型' })
  businessType!: string;
  ...
  @Column({ type: 'varchar', length: 50, comment: '操作: view/download/preview/export' })
  action!: string;
  ...
  @Column({ type: 'varchar', length: 255, nullable: true, comment: '原因/备注' })
  reason!: string | null;
